...
export const api = {
  articles: {
    ...
    getAllAdmin: "articles.getAllAdmin",
...
// Newsroom agents write articles with status "draft" (see newsroom/); those
// must never reach public surfaces. Anything without the field is live.
const isPublic = (a) => a.status !== "draft";
...
function subscribe(path, args, cb) {
  switch (path) {
    ...
    case api.articles.getAll: {
      ...
      return onSnapshot(q, (snap) => cb(snap.docs.map(normalize).filter(isPublic)));
    }

    // Admin view: everything, drafts included.
    case api.articles.getAllAdmin: {
      const q = query(collection(db, "articles"), orderBy("date", "desc"));
      return onSnapshot(q, (snap) => cb(snap.docs.map(normalize)));
    }

    // Single article by doc id — `undefined` while loading, `null` if missing
    // (used by the /a/[id] deep-link page).
    case api.articles.getById: {
      const { id } = args || {};
      if (!id) return () => {};
      return onSnapshot(doc(db, "articles", id), (snap) =>
        cb(snap.exists() ? normalize(snap) : null)
      );
    }
    ...
    case api.articles.getAllTags: {
      ...
      return onSnapshot(q, (snap) => {
        const tags = [
          ...new Set(
            snap.docs.map(normalize).filter(isPublic).map((a) => a.tag).filter(Boolean)
          ),
        ].sort();
        cb(tags);
      });
    ...
    case api.articles.getBookmarked: {
      ...
      const unsubA = onSnapshot(
        ...
        (snap) => {
          articlesCache = snap.docs.map(normalize).filter(isPublic);
          emit();
        }
      );
